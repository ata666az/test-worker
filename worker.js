// ====================================================================
// WORKER FINAL v2 — Load Balancer + VLESS + TCP + UDP Relay + MUX
//                    + Chained Proxy via Path
// ====================================================================
//
// Perbaikan v2:
//   - Hapus AbortController sepenuhnya (penyebab error 1101/500)
//   - Manual race tanpa Promise.any (hindari unhandled rejection)
//   - Setiap fetch() pakai .then(onSuccess, onError) eksplisit
// ====================================================================
...
// ====================================================================
// LOAD BALANCER HTTP — TANPA AbortController
// ====================================================================
    ...
    for (let i = 0; i < shuffled.length; i += batchSize) {
        ...
        const result = await new Promise((resolve) => {
            ...
            for (const host of batch) {
                ...
                if (hasBody) {
                    try {
                        options.body = request.clone().body;
                    } catch (_) {
                        pending--;
